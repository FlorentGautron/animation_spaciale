...
function planeteAnimation(id) {
    ...
    if(div.classList.contains('class_div_img')) {
        ...
        // AFFICHER LE TEXTE
        let divPlanete = document.getElementById('divImg' +id);
        let titrePlanete = document.getElementById('titre_planete_' +id);
        let ulPlanete = document.getElementById('ul_planete_' +id);
        let hrefPlanete = document.getElementById('idHrefSavoirplus_' +id);

        setTimeout(function(){
            divPlanete.classList.remove('hide');
            ulPlanete.classList.remove('hide');
            titrePlanete.classList.remove('hide');
            hrefPlanete.classList.remove('hide');
...
image_font_space.addEventListener('click', function() {
    ...
    if( divs.length > 0 && divs[0].dataset.actif === '1'){
        ...
        // AFFICHER LE TEXTE
        let divTextePlanete = document.getElementById('div_texte_planete_' +id);
        let titrePlanete = document.getElementById('titre_planete_' +id);
        let ulPlanete = document.getElementById('ul_planete_' +id);
        let hrefPlanete = document.getElementById('idHrefSavoirplus_' +id);

        divTextePlanete.classList.add('hide');
        titrePlanete.classList.add('hide');
        ulPlanete.classList.add('hide');
        hrefPlanete.classList.add('hide');
        titrePlanete.style.fontSize = '1em';
    }
})
